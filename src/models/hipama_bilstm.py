...
class HiPAMA(nn.Module):
    def __init__(self, embed_dim, depth, input_dim=84, num_heads=4):
        ...
        # self.gru = torch.nn.GRU(input_size=embed_dim, hidden_size=embed_dim, num_layers=depth, batch_first=True)
        self.bilstm = torch.nn.LSTM(input_size=embed_dim, hidden_size=embed_dim, num_layers=depth, batch_first=True, bidirectional=True)
        self.combine_linear = nn.Linear(2 * embed_dim, embed_dim)  # New: Learnable combination linear layer
        self.attn = Attention(
    ...
    def forward(self, x, phn):
        ...
        x, _ = self.bilstm(x)
        x_combined = self.combine_linear(x)
        x = x_combined
